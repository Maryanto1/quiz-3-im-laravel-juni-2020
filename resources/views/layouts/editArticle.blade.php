@extends('master')
@section('judul', 'Edit Answers Table')
@section('mainContent')
<div>
    <form action="/jawaban/{{ $datas[0]->id }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group row">
          <label for="inputEmail2" class="col-sm-2 col-form-label">Isi Jawaban</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" id="inputEmail2" value="{{ $datas[0]->isi }}" name="isi">
          </div>
        </div>
